...
// Cara frontal de la pirámide
void CrearCaraFrontal()
{
...
// Cara derecha de la pirámide
void CrearCaraDerecha()
{
	unsigned int indices[] = {
		0, 1, 2
	};
...
// Cara trasera de la pirámide
void CrearCaraTrasera()
{
	unsigned int indices[] = {
		0, 1, 2
	};
...
// Cara izquierda de la pirámide
void CrearCaraIzquierda()
{
	unsigned int indices[] = {
		0, 1, 2
	};
...
// Cara cuadrada de la pirámide
void CrearBasePiramide()
{
	unsigned int indices[] = {
		0, 1, 2,
		2, 3, 0
	...
	Mesh* base = new Mesh();
	base->CreateMesh(vertices, indices, 12, 6);
...
int main()
{
	...
	CrearCilindro(20, 0.5f);//índice 1 en MeshList
	CrearCono(20, 0.7f);//índice 2 en MeshList
	CrearPiramideCuadrangular();//índice 3 en MeshList

	CrearCaraFrontal();        // índice 4
	CrearCaraDerecha();        // índice 5
	CrearCaraTrasera();        // índice 6
	CrearCaraIzquierda();      // índice 7
	CrearBasePiramide();       // índice 8
	CreateShaders();
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::translate(model, glm::vec3(0.0f, 0.35f, -3.48f));

		model = glm::scale(model, glm::vec3(0.35f, 0.35f, 0.20f));
		...
		model = glm::translate(model, glm::vec3(-0.85f, -0.45f, -4.0f));
		...
		model = glm::scale(model, glm::vec3(0.7f, 0.18f, 1.0f));
		...
		model = glm::scale(model, glm::vec3(0.35f, 0.05f, 1.0f));
		...
		// SOPORTE MOTOR IZQUIERDO - CUBO
		model = glm::mat4(1.0f);

		model = glm::translate(model, glm::vec3(-0.35f, -1.25f, -4.0f));

		model = glm::scale(model, glm::vec3(0.15f, 0.35f, 0.65f));
		...
		model = glm::translate(model, glm::vec3(0.0f, -1.45f, -4.0f));
